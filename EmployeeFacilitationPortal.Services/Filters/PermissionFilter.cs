...
{
    public class PermissionsFilter : IAuthorizationPolicyProvider
    {
        const string POLICY_PREFIX = "Permissions";
        public DefaultAuthorizationPolicyProvider FallbackPolicyProvider { get; }

        public PermissionsFilter(IOptions<AuthorizationOptions> options)
        {
        ...
        public Task<AuthorizationPolicy> GetPolicyAsync(string policyName)
        {
            ...
            {
                var pageUrl = policyName.Substring(POLICY_PREFIX.Length);
                if (pageUrl != "" || pageUrl != null)
                {
                    var policy = new AuthorizationPolicyBuilder();
                    policy.AddRequirements(new PagePermissionUrl(pageUrl));
